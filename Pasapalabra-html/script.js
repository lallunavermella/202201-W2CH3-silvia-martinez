window.addEventListener("load", () => {
  ...
  const radius = 200;
  for (let i = 0; i < 27; i++) {
    const x = radius * Math.cos(degreesToRadians(-90 + i * stepDeg));
    const y = radius * Math.sin(degreesToRadians(-90 + i * stepDeg));
  ...
  letters.forEach((letter) => {
    const currentLetter = letter.getAttribute("value");
  ...
  function nextQuestion(questions) {
    index = questions.findIndex(
      (element) => element.status === 0 && element.pasapalabra === false
    );
    if (index === -1) {
      for (let i = 0; i < questions.length; i++) {
        questions[i].pasapalabra = false;
  ...
  let checkPasapalabra = (answer) => answer === "pasapalabra" || answer === "";

  let checkEnd = (answer) => answer === "end";
